...
class AIRecommendationSystem:
    ...
    def record_user_interaction(self, user_id: str, media_id: str, interaction_type: str, 
                              interaction_value: float = 1.0, metadata: Dict[str, Any] = None):
        """
        记录用户行为交互
        ...
        try:
            if self.conn:
                ...
                self.cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (user_id, media_id, interaction_type, interaction_value, metadata_str))
                self.conn.commit()
    ...
    def _update_user_preferences(self, user_id: str, media_id: str, interaction_type: str, value: float):
        """根据用户交互更新用户偏好"""
        try:
            # 查找媒体信息
            media_item = None
            ...
            if media_item.get("genres"):
                for genre in media_item["genres"]:
                    self._update_preference_weight(user_id, "genre", genre, value * weight_multiplier)
            
            # 更新导演偏好
            if media_item.get("directors"):
                for director in media_item["directors"]:
                    self._update_preference_weight(user_id, "director", director, value * weight_multiplier)
            
            # 更新演员偏好
            if media_item.get("actors"):
                for actor in media_item["actors"][:3]:  # 只取前3个主要演员
                    self._update_preference_weight(user_id, "actor", actor, value * weight_multiplier)
            
            # 更新类型偏好
            if media_item.get("type"):
                self._update_preference_weight(user_id, "media_type", media_item["type"], value * weight_multiplier)
            
            # 更新音乐特定偏好（艺术家、专辑等）
            if media_item.get("type") == "music":
                if media_item.get("artist"):
                    self._update_preference_weight(user_id, "music_artist", media_item["artist"], value * weight_multiplier)
                if media_item.get("album"):
                    self._update_preference_weight(user_id, "music_album", media_item["album"], value * weight_multiplier)
                if media_item.get("genre"):
                    self._update_preference_weight(user_id, "music_genre", media_item["genre"], value * weight_multiplier)
            
        except Exception as e:
            logger.error(f"更新用户偏好失败: {e}")
    
    def _update_preference_weight(self, user_id: str, pref_type: str, pref_value: str, weight_delta: float):
        """更新偏好权重"""
        try:
            if self.conn:
                # 检查是否已存在偏好
            ...
            self.user_preferences[user_id][pref_key] = max(0.1, current_weight + weight_delta)
            
        except Exception as e:
            logger.error(f"更新偏好权重失败: {e}")
    
    def get_personalized_recommendations(self, user_id: str, query_text: str = None, 
        ...
        try:
            ...
            if query_text:
                ...
                # 如果没有查询文本，基于用户历史推荐
                base_results = self._get_recommendations_from_history(user_id, top_k * 2)
            
            if not base_results:
                return []
            
            # 应用个性化权重
            personalized_results = self._apply_personalization_weights(user_id, base_results)
    ...
    def _get_recommendations_from_history(self, user_id: str, top_k: int) -> List[Dict[str, Any]]:
        """基于用户历史获取推荐"""
        try:
            # 获取用户最近交互的媒体
            ...
            for interaction in recent_interactions:
                similar_items = self.get_similar_to_media(interaction["media_id"], top_k // 2)
    ...
    def _get_popular_recommendations(self, top_k: int) -> List[Dict[str, Any]]:
        ...
        try:
            # 按评分排序
            sorted_media = sorted(self.media_items, 
    ...
    def _apply_personalization_weights(self, user_id: str, recommendations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """应用个性化权重到推荐结果"""
        try:
            ...
            for item in recommendations:
                ...
                if item.get("genres"):
                    for genre in item["genres"]:
                        ...
                        personalization_score += self.user_preferences[user_id].get(pref_key, 0.1)
                ...
                # 计算综合得分
                personalized_score = base_score + (personalization_score * 0.1) + diversity_bonus + rating_bonus + recency_bonus
    ...
    def generate_media_embedding(self, media_data: Dict[str, Any]) -> np.ndarray:
        ...
        if media_data.get("genres"):
            genres = ", ".join(media_data["genres"]) if isinstance(media_data["genres"], list) else media_data["genres"]
        ...
        if media_data.get("directors"):
            directors = ", ".join(media_data["directors"]) if isinstance(media_data["directors"], list) else media_data["directors"]
            text_parts.append(f"导演: {directors}")
        if media_data.get("actors"):
            actors = ", ".join(media_data["actors"]) if isinstance(media_data["actors"], list) else media_data["actors"]
    ...
    def get_similar_items(self, query_text: str, top_k: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            # 使用FAISS进行高效搜索
            distances, indices = self.faiss_index.search(query_embedding, top_k * 2)  # 搜索更多结果用于过滤
            ...
            for i, (distance, idx) in enumerate(zip(distances[0], indices[0])):
                if idx < len(self.media_items):  # 确保索引有效
                    ...
                    # 过滤低相似度结果
                    if similarity >= self.config["similarity_threshold"] or len(results) < top_k:
                        media_item = self.media_items[idx].copy()
                        media_item["similarity_score"] = similarity
                        media_item["rank"] = len(results) + 1
                        media_item["confidence"] = "high" if similarity >= self.config["similarity_threshold"] else "low"
    ...
    def _fallback_similar_items(self, query_text: str, top_k: int) -> List[Dict[str, Any]]:
        """回退到传统相似度计算方法"""
        try:
            query_embedding = self.generate_embedding(query_text)
    ...
    def get_similar_to_media(self, media_id: str, top_k: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        基于特定媒体内容获取相似推荐
    ...
    def batch_recommend(self, media_ids: List[str], top_k: int = 5) -> Dict[str, List[Dict[str, Any]]]:
        """
        批量推荐
