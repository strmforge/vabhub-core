...
class UserFeedbackRequest(BaseModel):
    ...
    feedback_type: str = Field(..., description="反馈类型: like/dislike/neutral", regex="^(like|dislike|neutral)$")
...
async def recommend_by_query(
    ...
    try:
        ...
        # 获取推荐
        recommendations = recommender.get_similar_items(
            request.query, 
            request.top_k
        )
...
@router.post("/media/{media_id}", response_model=RecommendationResponse, summary="基于媒体推荐")
async def recommend_by_media(
    media_id: str,
    request: RecommendationRequest = Body(...)
):
    """
    ...
    try:
        ...
        # 获取推荐
        recommendations = recommender.get_similar_to_media(
            media_id, 
            request.top_k
        )
...
async def submit_user_feedback(
    feedback: UserFeedbackRequest = Body(...)
):
    """
    提交用户对推荐结果的反馈
...
async def set_user_preferences(
    preferences: UserPreferenceRequest = Body(...)
):
    """
    设置用户个性化偏好
...
@router.post("/personalized/{user_id}", response_model=RecommendationResponse, summary="个性化推荐")
        ...
        response = RecommendationResponse(
            ...
            stats={
                **recommender.get_recommendation_stats(),
                ...
                "personalization_score": sum(item.get("personalized_score", 0) for item in personalized_recommendations) / len(personalized_recommendations) if personalized_recommendations else 0
            },
            timestamp=datetime.now()
        )
