...
from .exceptions import (
    VabHubException, DatabaseException, CacheException, AuthenticationException,
    AuthorizationException, ValidationException, NotFoundException,
    ExternalServiceException, RateLimitException, PluginException,
    exception_handler
)
...
class VabHubAPI:
    ...
    async def _log_startup_info(self):
        """记录API启动信息"""
        self.logger.info("VabHub Core API 启动成功", extra={
            "version": "1.5.0",
    ...
    def _setup_routes(self):
        ...
        @self.app.post("/api/subscriptions", response_model=Subscription, status_code=201)
        ...
        async def test_sub(query: str = Query(..., description="query or rule to test")):
        ...
        async def test_scraper(q: str = Query(...)):
            return {"ok": True, "candidates": [{"provider": "tmdb", "id": "550", "title": "Fight Club"}]}
        ...
        async def strm_emit(req: STRMEmitRequest):
            try:
                ...
                raise HTTPException(status_code=500, detail=f"Failed to generate STRM file: {e}")
        ...
        async def list_strm_files():
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail=f"Failed to scan STRM files: {e}")
        ...
        async def organize_strm_files(rules: Dict[str, Any] = {}):
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail=f"Failed to organize STRM files: {e}")
        
        @self.app.post("/api/strm/validate")
        async def validate_strm_file(file_path: str):
            """Validate STRM file"""
            try:
                validation_result = self.strm_gateway_manager.validate_strm_file(file_path)
                return {"ok": True, "validation": validation_result}
            except Exception as e:
                raise HTTPException(status_code=500, detail=f"Failed to validate STRM file: {e}")
        ...
        async def batch_generate_strm_files(media_list: List[Dict[str, Any]]):
            ...
            try:
                results = self.strm_gateway_manager.batch_generate_strm_files(media_list)
                return {"ok": True, "results": results}
            except Exception as e:
                raise HTTPException(status_code=500, detail=f"Failed to batch generate STRM files: {e}")
        
        @self.app.post("/api/strm/cleanup")
        async def cleanup_old_strm_files(max_age_days: int = 30):
            """Clean up old STRM files"""
            try:
                cleanup_result = self.strm_gateway_manager.cleanup_old_strm_files(max_age_days)
                return {"ok": True, "cleanup": cleanup_result}
            except Exception as e:
                raise HTTPException(status_code=500, detail=f"Failed to cleanup old STRM files: {e}")
        
        # Plugins management
        @self.app.get("/api/plugins/registry")
        async def plugins_registry():
            return {"official": [{"id": "strm-emitter", "version": "0.0.1"}], "community": []}
        ...
        async def get_charts(
            ...
            """获取图表数据"""
            try:
                charts = await self.charts_service.fetch_charts(source, region, time_range, media_type, limit)
                return charts
            except HTTPException:
                raise
            except Exception as e:
        ...
        async def get_chart_sources():
            ...
            return {
                "sources": [
                    {"id": "tmdb", "name": "TMDB", "supported_regions": ["US", "CN", "JP", "KR"], "media_types": ["movie", "tv"]},
                    {"id": "spotify", "name": "Spotify", "supported_regions": ["US", "GB", "JP", "CN"], "media_types": ["music"]},
                    {"id": "apple_music", "name": "Apple Music", "supported_regions": ["US", "GB", "JP", "CN"], "media_types": ["music"]},
                    {"id": "bangumi", "name": "Bangumi", "supported_regions": ["JP"], "media_types": ["anime"]}
                ]
            }
