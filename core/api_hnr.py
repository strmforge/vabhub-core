...
async def batch_detect_hnr(
    request: BatchHNRDetectionRequest,
    current_user: dict = Depends(get_current_user)
):
    """批量检测HNR风险"""
    try:
        ...
        summary = {
            ...
            "passed": passed_count,
            "blocked_percentage": blocked_count / len(request.candidates) * 100 if request.candidates else 0
        }
        
        return BatchHNRDetectionResponse(
            results=results,
            summary=summary
        )
