...
async def sanitize_filename(request: SanitizeRequest):
    ...
    return SanitizeResponse(original=request.filename, sanitized=sanitized)
...
async def get_unique_filename(request: UniqueFilenameRequest):
    ...
    return UniqueFilenameResponse(
        original=request.filepath, unique_path=str(unique_path)
    )
...
async def organize_file(request: OrganizeRequest):
    """根据媒体类型组织文件"""
    organized_path = path_manager.organize_by_type(request.filepath, request.media_type)
    return OrganizeResponse(
        original=request.filepath, organized_path=str(organized_path)
    )
...
async def create_symlink(request: SymlinkRequest):
    ...
    success = path_manager.create_symlink(
        request.source, request.destination, request.hard_link
    )

    if success:
        return SymlinkResponse(success=True, message="链接创建成功")
    else:
