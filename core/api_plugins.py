...
async def get_plugin_config(plugin_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get plugin config: {str(e)}"
        )
...
async def update_plugin_config(plugin_id: str, config_update: PluginConfigUpdate):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update plugin config: {str(e)}"
        )
...
async def execute_plugin_method(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to execute plugin method: {str(e)}"
        )
