...
async def rename_file(
    ...
    try:
        ...
        if success:
            return {"message": "File renamed successfully", "old_path": old_path, "new_filename": new_filename}
        else:
...
async def organize_media_file(
    ...
    try:
        ...
        results = organizer.organize_media_file(file_path, media_info, url, generate_strm)
        return {"file_path": file_path, "results": results}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to organize media file: {e}")
