...
class MemoryCacheBackend(CacheBackend):
    ...
    async def _evict(self):
        ...
        if self.policy == CachePolicy.LRU:
            ...
            candidates = [k for k, v in self._creation_times.items() if v == oldest_time]
            if candidates:
                await self.delete(candidates[0])
...
class RedisCacheBackend(CacheBackend):
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379", prefix: str = "vabhub:",
                 max_connections: int = 10, health_check_interval: int = 30):
        if redis is None:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 检查连接池状态
            pool_info = {
                'connections': self.redis_pool._in_use_connections + self.redis_pool._available_connections,
...
class CacheManager:
    ...
    async def get_stats(self, level: Optional[CacheLevel] = None) -> Dict[CacheLevel, CacheStats]:
        """获取缓存统计信息"""
        if level:
    ...
    async def batch_get(self, keys: List[str], level: Optional[CacheLevel] = None) -> Dict[str, Any]:
        """批量获取缓存值"""
        levels_to_check = [level] if level else self.enabled_levels
    ...
    async def batch_delete(self, keys: List[str], level: Optional[CacheLevel] = None) -> bool:
        """批量删除缓存值"""
        levels_to_delete = [level] if level else self.enabled_levels
        results = []
        ...
        for cache_level in levels_to_delete:
            ...
            if hasattr(backend, 'batch_delete'):
                ...
                if success:
                    self.stats[cache_level].size = max(0, self.stats[cache_level].size - len(keys))
            else:
                # 如果没有批量操作支持，回退到单个删除
                for key in keys:
                    success = await backend.delete(key)
    ...
    async def get_memory_usage(self, level: Optional[CacheLevel] = None) -> Dict[str, Any]:
        """获取内存使用情况"""
        levels_to_check = [level] if level else self.enabled_levels
    ...
    async def _update_other_levels(self, key: str, value: Any, source_level: CacheLevel):
        """更新其他级别的缓存"""
