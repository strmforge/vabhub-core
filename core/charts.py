...
class ChartsService:
    ...
    async def fetch_charts(self, 
        ...
        # 根据数据源调用不同的provider
        if source == "tmdb":
            charts = await self._fetch_tmdb_charts(region, time_range, media_type, limit)
        elif source == "spotify":
    ...
    async def _fetch_tmdb_charts(self, region: str, time_range: str, media_type: str, limit: int) -> List[ChartItem]:
        """获取TMDB图表数据"""
        try:
            if not self.config.TMDB_API_KEY:
                # 如果没有API密钥，返回模拟数据
                return await self._fetch_tmdb_mock_data(region, time_range, media_type, limit)
    ...
    async def _fetch_tmdb_mock_data(self, region: str, time_range: str, media_type: str, limit: int) -> List[ChartItem]:
        """获取TMDB模拟数据"""
    ...
    async def _fetch_spotify_charts(self, region: str, time_range: str, limit: int) -> List[ChartItem]:
        """获取Spotify图表数据"""
        try:
            # 模拟Spotify API调用
    ...
    async def _fetch_apple_music_charts(self, region: str, time_range: str, limit: int) -> List[ChartItem]:
        """获取Apple Music图表数据"""
        try:
            # 模拟Apple Music API调用
            await asyncio.sleep(0.1)
            ...
            raise HTTPException(status_code=500, detail=f"Apple Music API error: {str(e)}")
    
    async def _fetch_bangumi_charts(self, time_range: str, limit: int) -> List[ChartItem]:
        """获取Bangumi图表数据"""
        try:
            # 模拟Bangumi API调用
            await asyncio.sleep(0.1)
