...
class QbittorrentClient(DownloadClient):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            await asyncio.get_event_loop().run_in_executor(
                None, 
                lambda: self._client.auth_log_in()
            )
    ...
    async def disconnect(self):
        ...
        if self._client:
            try:
                await asyncio.get_event_loop().run_in_executor(
                    None, 
                    lambda: self._client.auth_log_out()
                )
            except Exception as e:
    ...
    async def add_torrent(self, torrent: Union[str, bytes], save_path: str = None,
                         category: str = None, tags: List[str] = None, **kwargs) -> bool:
        """添加种子到qBittorrent"""
        ...
        try:
            ...
            logger.info(f"成功添加种子到qBittorrent: {torrent if isinstance(torrent, str) else 'torrent file'}")
            return True
    ...
    async def get_torrents(self, status_filter: Optional[TorrentStatus] = None,
                          category: Optional[str] = None) -> List[TorrentInfo]:
        """获取种子列表"""
        if not self._connected:
            return []
        ...
        try:
            ...
            torrents = await asyncio.get_event_loop().run_in_executor(
                None, 
                lambda: self._client.torrents_info(**params)
            )
    ...
    async def set_speed_limit(self, torrent_hash: str, download_limit: int = 0,
                             upload_limit: int = 0) -> bool:
        """设置速度限制"""
        if not self._connected:
            return False
    ...
    async def get_transfer_info(self) -> Dict[str, Any]:
        ...
        try:
            transfer_info = await asyncio.get_event_loop().run_in_executor(
                None, 
                lambda: self._client.transfer_info()
            )
        ...
        try:
            ...
            if connected:
                version = await asyncio.get_event_loop().run_in_executor(
                    None, 
                    lambda: self._client.app_version()
                )
                return {
                    'ok': True,
                    'version': version,
