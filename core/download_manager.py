...
from .download_client import (
    DownloadClient, DownloadClientFactory, DownloadClientConfig, 
    DownloadClientType, TorrentInfo, TorrentStatus
)
import logging
...
class DownloadManager:
    ...
    async def add_client(self, 
        ...
        try:
            ...
            if not test_result.get('ok', False):
                logger.error(f"下载器连接测试失败: {test_result.get('error', 'Unknown error')}")
                return False
    ...
    async def remove_client(self, client_id: str) -> bool:
        ...
        try:
            ...
            if self._default_client == client_id:
                self._default_client = next(iter(self._clients.keys()), None) if self._clients else None
    ...
    async def get_torrents(self,
                          ...
                          category: Optional[str] = None) -> List[TorrentInfo]:
        """获取种子列表"""
        if client_id:
            # 获取指定客户端的种子
    ...
    async def get_torrent(self, 
                         torrent_hash: str,
                         client_id: Optional[str] = None) -> Optional[TorrentInfo]:
        """获取单个种子信息"""
        if client_id:
            # 在指定客户端中查找
            client = self.get_client(client_id)
    ...
    async def set_ratio_limit(self, 
                             torrent_hash: str,
                             ratio: float,
                             client_id: Optional[str] = None) -> bool:
        """设置分享率限制"""
    ...
    async def set_speed_limit(self, 
                             torrent_hash: str,
        ...
        return await client.set_speed_limit(
            torrent_hash, download_limit, upload_limit
        )
    ...
    def _get_target_client(self, client_id: Optional[str] = None) -> Optional[DownloadClient]:
        """获取目标客户端"""
        if client_id:
