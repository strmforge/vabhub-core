...
class FileOrganizer:
    ...
    def _extract_metadata(self, filename: str) -> Dict[str, Any]:
        ...
        # 提取分辨率
        resolution_match = re.search(r'(480p|720p|1080p|4K|2160p)', filename, re.IGNORECASE)
        if resolution_match:
            metadata['resolution'] = resolution_match.group().lower()
        
        # 提取编码信息
        codec_match = re.search(r'(H\.?264|H\.?265|HEVC|AVC|X264|X265)', filename, re.IGNORECASE)
        if codec_match:
        ...
        title = re.sub(r'S\d{1,2}E\d{1,2}', '', title, flags=re.IGNORECASE)  # 移除季集信息
        title = re.sub(r'(480p|720p|1080p|4K|2160p)', '', title, flags=re.IGNORECASE)  # 移除分辨率
        title = re.sub(r'(H\.?264|H\.?265|HEVC|AVC|X264|X265)', '', title, flags=re.IGNORECASE)  # 移除编码
        title = re.sub(r'(DTS|AC3|AAC|FLAC|MP3)', '', title, flags=re.IGNORECASE)  # 移除音频
    ...
    def organize_file(self, file_info: FileInfo, rule: OrganizationRule = None) -> Dict[str, Any]:
        """整理单个文件"""
        try:
            # 如果没有指定规则，自动匹配规则
            if not rule:
    ...
    def _perform_file_action(self, source_path: str, target_path: str, action: FileAction) -> bool:
        """执行文件操作"""
        ...
        try:
            ...
            logger.error(f"Error performing {action.value} from {source} to {target}: {e}")
            return False
