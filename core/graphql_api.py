...
class SubscriptionManager:
    ...
    async def register_subscription(self, subscription_id: str, websocket: WebSocket, metadata: Dict[str, Any] = None):
        """注册新的订阅连接"""
        async with self.lock:
            ...
            logger.debug(f"Subscription registered: {subscription_id} with metadata: {metadata}")

    async def unsubscribe_from_channel(self, subscription_id: str, channel: str):
        """取消订阅广播频道"""
        async with self.lock:
            if channel in self.broadcast_channels and subscription_id in self.broadcast_channels[channel]:
                self.broadcast_channels[channel].remove(subscription_id)
                logger.debug(f"Subscription {subscription_id} unsubscribed from channel {channel}")
                # 更新统计信息
                if channel in self.stats["channels"] and self.stats["channels"][channel] > 0:
    ...
    async def subscribe_to_channel(self, subscription_id: str, channel: str):
        ...
        async with self.lock:
            ...
            if subscription_id not in self.broadcast_channels[channel]:
                ...
                self.stats["channels"][channel] += 1
                logger.debug(f"Subscription {subscription_id} subscribed to channel {channel}")
        ...
        for subscription_id in subscribers:
            if subscription_id in self.active_subscriptions:
                ...
                try:
                    ...
                    logger.debug(f"Message sent to subscription {subscription_id} on channel {channel}")
    ...
    async def get_cached_messages(self, channel: str, since_timestamp: Optional[float] = None) -> List[Dict[str, Any]]:
        """获取缓存的频道消息"""
        cached_messages = []
        current_time = time.time() * 1000
        ...
        for cache_key, cache_data in self.subscription_cache['by_channel'].items():
            if cache_key.startswith(f"channel:{channel}:"):
                ...
                if current_time - cache_data['timestamp'] < cache_data['ttl']:
                    # 检查时间戳过滤
                    if since_timestamp is None or cache_data['timestamp'] > since_timestamp:
...
class GraphQLWebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, connection_params: Dict[str, Any] = None):
        """建立WebSocket连接"""
            ...
            return {
                ...
                "graphql": {
                    ...
                    "cache": {
                        ...
                        "cached_messages": subscription_manager.stats["cached_messages"]
                    }
                }
            }
