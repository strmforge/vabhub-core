...
class EnhancedMediaServerManager:
    ...
    async def add_server(self, server_config: MediaServerConfig) -> bool:
        ...
        try:
            ...
            if not test_result["ok"]:
                logger.error(f"Failed to connect to server {server_config.name}: {test_result['error']}")
                return False
            ...
            logger.info(f"Added media server: {server_config.name} ({server_config.server_type.value})")
            return True
    ...
    async def test_connection(self, server_config: MediaServerConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            elif server_config.server_type in [MediaServerType.EMBY, MediaServerType.JELLYFIN]:
                return await self._test_emby_connection(server_config)
            else:
                return {"ok": False, "error": f"Unsupported server type: {server_config.server_type}"}
        except Exception as e:
            return {"ok": False, "error": str(e)}
    
    async def _test_plex_connection(self, server_config: MediaServerConfig) -> Dict[str, Any]:
        """测试Plex连接"""
        try:
            headers = {"X-Plex-Token": server_config.api_key}
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{server_config.url}/library/sections", headers=headers)
    ...
    async def _test_emby_connection(self, server_config: MediaServerConfig) -> Dict[str, Any]:
        """测试Emby/Jellyfin连接"""
        try:
            headers = {"X-Emby-Token": server_config.api_key}
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{server_config.url}/System/Info", headers=headers)
    ...
    async def _get_plex_libraries(self, server_config: MediaServerConfig) -> List[LibraryInfo]:
        """获取Plex媒体库"""
        headers = {"X-Plex-Token": server_config.api_key}
    ...
    async def _get_emby_libraries(self, server_config: MediaServerConfig) -> List[LibraryInfo]:
        """获取Emby/Jellyfin媒体库"""
        headers = {"X-Emby-Token": server_config.api_key}
        
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{server_config.url}/Library/VirtualFolders", headers=headers)
    ...
    async def refresh_library(self, server_id: str, library_name: str) -> Dict[str, Any]:
        """刷新媒体库"""
        if server_id not in self.servers:
    ...
    async def _refresh_plex_library(self, server_config: MediaServerConfig, library_name: str) -> Dict[str, Any]:
        """刷新Plex媒体库"""
        headers = {"X-Plex-Token": server_config.api_key}
    ...
    async def _refresh_emby_library(self, server_config: MediaServerConfig, library_name: str) -> Dict[str, Any]:
        """刷新Emby/Jellyfin媒体库"""
        headers = {"X-Emby-Token": server_config.api_key}
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 204:
                    return {"ok": True, "message": f"Library {library_name} refresh initiated"}
                else:
                    return {"ok": False, "error": f"HTTP {response.status_code}: {response.text}"}
        except Exception as e:
            return {"ok": False, "error": str(e)}
    
    async def search_media(self, query: str, media_type: str = "All", server_ids: List[str] = None) -> List[MediaItem]:
        """在所有服务器中搜索媒体"""
    ...
    async def _search_server_media(self, server_id: str, query: str, media_type: str) -> List[MediaItem]:
        """在单个服务器中搜索媒体"""
        server_config = self.servers[server_id]
    ...
    async def _search_plex_media(self, server_config: MediaServerConfig, query: str, media_type: str) -> List[MediaItem]:
        """搜索Plex媒体"""
        headers = {"X-Plex-Token": server_config.api_key}
    ...
    async def _search_emby_media(self, server_config: MediaServerConfig, query: str, media_type: str) -> List[MediaItem]:
        """搜索Emby/Jellyfin媒体"""
        headers = {"X-Emby-Token": server_config.api_key}
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                response = await client.get(
                    f"{server_config.url}/Search/Hints",
                    headers=headers,
                    params=params
                )
    ...
    async def get_recently_added(self, server_id: str, limit: int = 10) -> List[MediaItem]:
        """获取最近添加的媒体"""
        if server_id not in self.servers:
            return []
    ...
    async def _get_plex_recently_added(self, server_config: MediaServerConfig, limit: int) -> List[MediaItem]:
        """获取Plex最近添加的媒体"""
        headers = {"X-Plex-Token": server_config.api_key}
    ...
    async def _get_emby_recently_added(self, server_config: MediaServerConfig, limit: int) -> List[MediaItem]:
