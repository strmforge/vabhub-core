...
class MetadataProvider(ABC):
    ...
    @abstractmethod
    async def search(self, query: str, media_type: str = "movie", language: str = "zh-CN") -> List[MediaEntity]:
        """搜索媒体"""
        pass
    
    @abstractmethod
    async def get_movie(self, movie_id: str, language: str = "zh-CN") -> Optional[Movie]:
        """获取电影详情"""
        pass
    
    @abstractmethod
    async def get_tv_show(self, tv_id: str, language: str = "zh-CN") -> Optional[TVShow]:
        """获取电视剧详情"""
        pass
    
    @abstractmethod
    async def get_season(self, tv_id: str, season_number: int, language: str = "zh-CN") -> Optional[Season]:
        """获取季详情"""
        pass
    
    @abstractmethod
    async def get_episode(self, tv_id: str, season_number: int, episode_number: int, language: str = "zh-CN") -> Optional[Episode]:
        """获取剧集详情"""
        pass
...
class TMDBProvider(MetadataProvider):
    ...
    def _parse_episode(self, data: Dict[str, Any], tv_id: str, season_number: int) -> Episode:
        """解析剧集数据"""
        return Episode(
            id=str(data.get('id', '')),
...
class MetadataManager:
    ...
    async def search(self, query: str, media_type: str = "movie", language: str = "zh-CN") -> List[MediaEntity]:
        ...
        for provider_name in self.priority:
            if provider_name in self.providers:
                try:
                    results = await self.providers[provider_name].search(query, media_type, language)
    ...
    async def get_media(self, media_type: str, media_id: str, language: str = "zh-CN") -> Optional[MediaEntity]:
        """获取媒体详情"""
        for provider_name in self.priority:
            if provider_name in self.providers:
                try:
                    if media_type == MediaType.MOVIE:
                        result = await self.providers[provider_name].get_movie(media_id, language)
                    elif media_type == MediaType.TV_SHOW:
                        result = await self.providers[provider_name].get_tv_show(media_id, language)
                    else:
                        continue
    ...
    async def get_season(self, tv_id: str, season_number: int, language: str = "zh-CN") -> Optional[Season]:
        ...
        for provider_name in self.priority:
            if provider_name in self.providers:
                try:
                    result = await self.providers[provider_name].get_season(tv_id, season_number, language)
                    if result:
                        return result
                except Exception as e:
                    logging.error(f"Get season failed for provider {provider_name}: {e}")
    ...
    async def get_episode(self, tv_id: str, season_number: int, episode_number: int, language: str = "zh-CN") -> Optional[Episode]:
        ...
        for provider_name in self.priority:
            if provider_name in self.providers:
                try:
                    result = await self.providers[provider_name].get_episode(tv_id, season_number, episode_number, language)
                    if result:
                        return result
                except Exception as e:
                    logging.error(f"Get episode failed for provider {provider_name}: {e}")
