        ...
        async def wrapper(*args, **kwargs):
            for attempt in range(max_retries):
                try:
                    ...
                    logger.warning(f"重试 {attempt + 1}/{max_retries}: {e}, 等待 {delay} 秒")
    ...
    async def acquire(self):
        ...
        if len(self.calls) >= self.max_calls:
            ...
            if wait_time > 0:
                ...
                # 重新清理并检查
                self.calls = [call_time for call_time in self.calls if now + wait_time - call_time < self.period]
...
class MusicPlatformAdapter(abc.ABC):
    ...
    @abc.abstractmethod
    async def search_playlist(self, query: str, limit: int = 20) -> List[Dict[str, Any]]:
        """搜索歌单"""
        pass
    
    async def search(self, query: str, query_type: str, limit: int = 20) -> List[Dict[str, Any]]:
        """通用搜索方法"""
        if query_type == 'track':
    ...
    @abc.abstractmethod
    async def get_artist_tracks(self, artist_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        """获取艺术家的热门歌曲"""
        pass
...
class MusicPlatformFactory:
    ...
    @staticmethod
    def create_adapter(platform_name: str, config: Config, cache_manager: Optional[CacheManager] = None) -> MusicPlatformAdapter:
        """创建音乐平台适配器"""
        adapters = {
