...
class MusicSubscriptionManager:
    ...
    def _init_adapters(self):
        ...
        try:
            ...
            for platform in supported_platforms:
                adapter = MusicPlatformFactory.create_adapter(platform, self.config, self.cache_manager)
    ...
    async def search_music(self, query: str, platform: str = 'all', 
                          query_type: str = 'track', limit: int = 20) -> List[Dict[str, Any]]:
        """
        搜索音乐
    ...
    async def sync_subscriptions(self) -> Dict[str, Any]:
        ...
        for subscription_id, subscription in self.subscriptions.items():
            ...
            try:
                ...
                subscription['next_sync'] = current_time + self._get_sync_interval(subscription['schedule'])
    ...
    async def cleanup(self):
        ...
        for sub_id, sub in self.subscriptions.items():
            ...
            if (sub['last_sync'] and 
                (current_time - sub['last_sync']).days > 30 and 
                not sub['enabled']):
...
class MusicSubscriptionManagerFactory:
    ...
    @classmethod
    def get_instance(cls, config: Config, cache_manager: Optional[CacheManager] = None) -> MusicSubscriptionManager:
        """获取单例实例"""
    ...
    @classmethod
    def create_manager(cls, config: Config, cache_manager: Optional[CacheManager] = None) -> MusicSubscriptionManager:
        """创建新的管理器实例"""
