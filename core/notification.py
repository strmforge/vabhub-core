        ...
        try:
            ...
            async with session.post(url, data=data) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logging.error(f"ServerChan HTTP error: {response.status} - {error_text}")
                    return False
...
# 便捷函数
async def send_success_notification(manager: NotificationManager, title: str, message: str, metadata: Dict[str, Any] = None):
    """发送成功通知"""
...
async def send_error_notification(manager: NotificationManager, title: str, message: str, metadata: Dict[str, Any] = None):
    """发送错误通知"""
    msg = NotificationMessage(
        title=title,
        message=message,
...
async def send_warning_notification(manager: NotificationManager, title: str, message: str, metadata: Dict[str, Any] = None):
    """发送警告通知"""
    msg = NotificationMessage(
        title=title,
        message=message,
...
async def send_info_notification(manager: NotificationManager, title: str, message: str, metadata: Dict[str, Any] = None):
    """发送信息通知"""
    msg = NotificationMessage(
        title=title,
        message=message,
