...
class PathManager:
    ...
    def create_symlink(self, source: str, destination: str, hard_link: bool = False) -> bool:
        """
        创建符号链接或硬链接
    ...
    def batch_rename(self, files: List[Dict[str, Any]], template: str) -> List[Dict[str, Any]]:
        """
        批量重命名文件
...
class FileOrganizer:
    ...
    def _identify_media_type(self, filepath: Path) -> str:
        ...
        if filepath.suffix.lower() in video_extensions:
            # 进一步识别视频类型
            if any(keyword in filename for keyword in ['season', 's\\d', 'e\\d', 'episode']):
                return "tv"
            else:
                return "movie"
