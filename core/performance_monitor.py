...
class PerformanceMonitor:
    ...
    def __init__(self, history_size: int = 1000):
        ...
        self.metrics_history: Dict[MetricType, Deque[PerformanceMetric]] = defaultdict(lambda: deque(maxlen=history_size))
    ...
    async def record_metric(self, metric_type: MetricType, value: float, tags: Optional[Dict[str, str]] = None):
        """记录性能指标"""
        timestamp = time.time()
    ...
    async def get_metrics_history(self, metric_type: MetricType, limit: Optional[int] = None) -> List[PerformanceMetric]:
        """获取指标历史记录"""
        ...
        for metric_type, stats in self.stats.items():
            ...
            if metric_type == MetricType.CPU_USAGE:
                if stats.avg_value > 80:
                    analysis["warnings"].append("CPU使用率过高，建议优化代码或增加计算资源")
                elif stats.avg_value > 60:
                    analysis["recommendations"].append("CPU使用率较高，建议监控并优化性能")
            
            elif metric_type == MetricType.MEMORY_USAGE:
                if stats.avg_value > 85:
                    analysis["warnings"].append("内存使用率过高，可能导致系统不稳定")
                elif stats.avg_value > 70:
                    analysis["recommendations"].append("内存使用率较高，建议优化内存使用")
            
            elif metric_type == MetricType.CACHE_HIT_RATE:
                if stats.avg_value < 70:
                    analysis["recommendations"].append("缓存命中率较低，建议优化缓存策略")
