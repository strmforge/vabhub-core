...
class FileRenamer:
    ...
    def __init__(self, base_path: str, template: str = None):
        ...
        self.template = RenameTemplate(template or "{title}.{year}.{SxxExx}.{codec}.{audio}")
    ...
    def generate_new_filename(self, old_filename: str, media_info: Dict[str, Any]) -> str:
        """生成新文件名"""
        ...
        # 确保扩展名有效
        if not extension or extension not in (self.video_extensions | self.audio_extensions):
            extension = '.mkv'  # 默认扩展名
        
        return f"{new_name}{extension}"
    
    def rename_file(self, old_path: str, new_filename: str, strategy: str = "move") -> bool:
        """重命名文件"""
...
class MediaOrganizer:
    ...
    def organize_media_file(self, file_path: str, media_info: Dict[str, Any], 
                          url: str = None, generate_strm: bool = False) -> Dict[str, str]:
        """组织单个媒体文件"""
        results = {}
        
        # 重命名文件
        new_filename = self.renamer.generate_new_filename(Path(file_path).name, media_info)
    ...
    def scan_and_organize(self, source_dir: str, target_dir: str = None) -> Dict[str, Dict[str, str]]:
        """扫描并组织目录中的媒体文件"""
        if target_dir:
        ...
        for file_path in source_path.rglob('*'):
            if file_path.is_file() and file_path.suffix.lower() in self.renamer.video_extensions:
                try:
