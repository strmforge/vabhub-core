...
class StorageManager:
        ...
        for storage_name, storage_config in storages_config.items():
            try:
                ...
                if config.validate():
                    ...
                    self.logger.info(f"Storage configured: {storage_name} ({storage_type.value})")
                else:
                    self.storage_status[storage_name] = StorageStatus.ERROR
                    self.logger.error(f"Storage config validation failed: {storage_name}")
            except Exception as e:
                self.logger.error(f"Failed to setup storage {storage_name}: {e}")
                self.storage_status[storage_name] = StorageStatus.ERROR
    
    async def upload_file(self, file_path: str, target_path: str, 
                         storage_name: str = None, **kwargs) -> Dict[str, Any]:
        """上传文件到存储"""
        try:
            ...
            # 选择存储
            selected_storage = await self._select_storage(storage_name, target_path, **kwargs)
            if not selected_storage:
                return {'error': 'No suitable storage available'}
            
            # 执行上传
            result = await self._upload_to_storage(selected_storage, file_path, target_path, **kwargs)
            
            # 更新存储使用情况
            await self._update_storage_usage(selected_storage, result.get('size', 0))
            
            self.logger.info(f"File uploaded: {file_path} -> {target_path} on {selected_storage}")
            return result
    ...
    async def download_file(self, source_path: str, target_path: str,
                           storage_name: str = None, **kwargs) -> Dict[str, Any]:
        """从存储下载文件"""
        try:
            # 选择存储
            selected_storage = await self._select_storage(storage_name, source_path, **kwargs)
            if not selected_storage:
                return {'error': 'No suitable storage available'}
            
            # 执行下载
            result = await self._download_from_storage(selected_storage, source_path, target_path, **kwargs)
            
            self.logger.info(f"File downloaded: {source_path} -> {target_path} from {selected_storage}")
            return result
    ...
    async def delete_file(self, file_path: str, storage_name: str = None, **kwargs) -> Dict[str, Any]:
        """删除存储中的文件"""
        try:
            # 选择存储
            selected_storage = await self._select_storage(storage_name, file_path, **kwargs)
            if not selected_storage:
                return {'error': 'No suitable storage available'}
            
            # 执行删除
            result = await self._delete_from_storage(selected_storage, file_path, **kwargs)
    ...
    async def list_files(self, path: str = '', storage_name: str = None, 
        ...
        try:
            ...
            # 执行列表操作
            result = await self._list_storage_files(selected_storage, path, recursive, **kwargs)
    ...
    async def get_file_info(self, file_path: str, storage_name: str = None, **kwargs) -> Dict[str, Any]:
        """获取文件信息"""
        try:
            # 选择存储
            selected_storage = await self._select_storage(storage_name, file_path, **kwargs)
            ...
            # 获取文件信息
            result = await self._get_storage_file_info(selected_storage, file_path, **kwargs)
    ...
    async def get_storage_stats(self, storage_name: str = None) -> Dict[str, Any]:
        ...
        try:
            if storage_name:
                ...
                status = self.storage_status.get(storage_name, StorageStatus.UNAVAILABLE)
                ...
                return {
                    ...
                    'available_size': config.max_size - config.used_size if config.max_size > 0 else float('inf'),
    ...
    async def _select_storage(self, storage_name: str = None, path: str = '', **kwargs) -> Optional[str]:
        """选择合适的存储"""
        try:
            # 如果指定了存储名称，直接使用
            if storage_name:
                if (storage_name in self.storage_configs and 
                    self.storage_status.get(storage_name) == StorageStatus.AVAILABLE):
                    return storage_name
                return None
            ...
            for name, config in self.storage_configs.items():
                if (config.enabled and 
                    self.storage_status.get(name) == StorageStatus.AVAILABLE):
    ...
    async def _upload_to_storage(self, storage_name: str, file_path: str, 
                                target_path: str, **kwargs) -> Dict[str, Any]:
        """上传文件到指定存储"""
        config = self.storage_configs[storage_name]
        
        if config.storage_type == StorageType.LOCAL:
            return await self._upload_to_local(storage_name, file_path, target_path, **kwargs)
        elif config.storage_type == StorageType.S3:
            return await self._upload_to_s3(storage_name, file_path, target_path, **kwargs)
        # 其他存储类型的上传实现...
        
        return {'error': f'Unsupported storage type: {config.storage_type.value}'}
    
    async def _upload_to_local(self, storage_name: str, file_path: str, 
                              target_path: str, **kwargs) -> Dict[str, Any]:
        """上传到本地存储"""
        try:
            config = self.storage_configs[storage_name]
    ...
    async def _upload_to_s3(self, storage_name: str, file_path: str, 
                          target_path: str, **kwargs) -> Dict[str, Any]:
        """上传到S3存储"""
        try:
            config = self.storage_configs[storage_name]
    ...
    async def _download_from_storage(self, storage_name: str, source_path: str,
                                    target_path: str, **kwargs) -> Dict[str, Any]:
        """从存储下载文件"""
        config = self.storage_configs[storage_name]
        
        if config.storage_type == StorageType.LOCAL:
            return await self._download_from_local(storage_name, source_path, target_path, **kwargs)
        elif config.storage_type == StorageType.S3:
            return await self._download_from_s3(storage_name, source_path, target_path, **kwargs)
        # 其他存储类型的下载实现...
        
        return {'error': f'Unsupported storage type: {config.storage_type.value}'}
    
    async def _download_from_local(self, storage_name: str, source_path: str,
                                 target_path: str, **kwargs) -> Dict[str, Any]:
        """从本地存储下载"""
        try:
            config = self.storage_configs[storage_name]
    ...
    async def _download_from_s3(self, storage_name: str, source_path: str,
                               target_path: str, **kwargs) -> Dict[str, Any]:
        """从S3存储下载"""
        try:
            config = self.storage_configs[storage_name]
    ...
    async def _delete_from_storage(self, storage_name: str, file_path: str, **kwargs) -> Dict[str, Any]:
        """从存储删除文件"""
        config = self.storage_configs[storage_name]
    ...
    async def _delete_from_local(self, storage_name: str, file_path: str, **kwargs) -> Dict[str, Any]:
        """从本地存储删除"""
        try:
            config = self.storage_configs[storage_name]
            storage_path = config.config.get('path', '')
    ...
    async def _delete_from_s3(self, storage_name: str, file_path: str, **kwargs) -> Dict[str, Any]:
        """从S3存储删除"""
        try:
            config = self.storage_configs[storage_name]
    ...
    async def _list_storage_files(self, storage_name: str, path: str = '',
                                 recursive: bool = False, **kwargs) -> Dict[str, Any]:
        """列出存储中的文件"""
        config = self.storage_configs[storage_name]
    ...
    async def _list_local_files(self, storage_name: str, path: str = '',
                               recursive: bool = False, **kwargs) -> Dict[str, Any]:
        """列出本地存储文件"""
        try:
            config = self.storage_configs[storage_name]
    ...
    async def _list_s3_files(self, storage_name: str, path: str = '',
                            recursive: bool = False, **kwargs) -> Dict[str, Any]:
        """列出S3存储文件"""
        try:
            config = self.storage_configs[storage_name]
    ...
    async def _get_storage_file_info(self, storage_name: str, file_path: str, **kwargs) -> Dict[str, Any]:
        """获取存储文件信息"""
        config = self.storage_configs[storage_name]
    ...
    async def _get_local_file_info(self, storage_name: str, file_path: str, **kwargs) -> Dict[str, Any]:
        """获取本地存储文件信息"""
        try:
            config = self.storage_configs[storage_name]
            storage_path = config.config.get('path', '')
    ...
    async def _get_s3_file_info(self, storage_name: str, file_path: str, **kwargs) -> Dict[str, Any]:
        """获取S3存储文件信息"""
        try:
            config = self.storage_configs[storage_name]
    ...
    def health_check(self) -> bool:
        ...
        try:
            # 检查是否有可用的存储
            available_storages = [name for name, status in self.storage_status.items()
