...
class SubscriptionManager:
    ...
    async def _check_subscriptions(self):
        ...
        for subscription in self.subscriptions.values():
            if (
                ...
                and subscription.next_check
                and subscription.next_check <= now
            ):

                try:
                    await self._process_subscription(subscription)
                    subscription.last_check = now
                    subscription.next_check = now + timedelta(
                        seconds=subscription.check_interval
                    )
                    subscription.updated_at = now
                except Exception as e:
                    logger.error(
                        f"Error processing subscription {subscription.name}: {e}"
                    )
    ...
    async def _should_download(
        self, item: Dict[str, Any], rules: List[SubscriptionRule]
    ) -> bool:
        """判断是否应该下载"""
        for rule in rules:
            if not rule.enabled:
    ...
    async def create_subscription(
        self, name: str, rules: List[SubscriptionRule], check_interval: int = 3600
    ) -> Subscription:
        """创建新订阅"""
