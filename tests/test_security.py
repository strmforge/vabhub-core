...
class TestSecurity:
    ...
    def test_sql_injection_protection(self, test_client):
        ...
        for attempt in sql_injection_attempts:
            # 测试参数注入
            response = test_client.get(f"/api/metadata/search?query={attempt}&media_type=movie")
    ...
    def test_input_validation(self, test_client):
        ...
        for invalid_input in invalid_inputs:
            if invalid_input is not None:
                # 测试搜索端点
                response = test_client.get(f"/api/metadata/search?query={invalid_input}&media_type=movie")
                # 应该返回422或200
                assert response.status_code in [200, 422]
